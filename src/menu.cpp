...
struct FilterChoice { int mode; const char* label; };
...
{
	for (int i = 0; i < kFilterCount; i++) if (kFilters[i].mode == mode) return i;
	return 0;
}
static const char* FilterLabel(int mode) { return kFilters[FilterIndex(mode)].label; }

// Guards against a config naming a mode the renderer does not implement.
static int MigrateFilter(int mode)
{
	for (int i = 0; i < kFilterCount; i++) if (kFilters[i].mode == mode) return mode;
	return 0;
}

static int  setting_filter = 0;    // renderer filter id; see kFilters

// >= 0 while the Controller page is waiting for the player to press the key or button
// they want bound. Holds the binding being rebound.
...
static const char* kNdsLayoutNames[] = {
	"Cima/Baixo", "Baixo/Cima", "Esq/Dir", "Dir/Esq",
	"So Cima", "So Baixo", "Hibrido C", "Hibrido B"
};
...
static const char* kNdsGapVals[] = { "0", "8", "16", "24", "32", "48", "64", "80", "96", "126" };
...
static int  setting_wallpaper = 1; // 0=None, 1=Static Noise, 2=Parallax Stars, 3=Cyber Grid
static bool setting_fullscreen = false;
static int  setting_theme = 0;     // 0=Red/Burgundy, 1=Blue, 2=Green, 3=Amber, 4=Gray, 5=Dark
static int  setting_deadzone = 1;  // 0=5%, 1=10%, 2=15%, 3=20%
static int  setting_latency = 1;   // 0=16ms, 1=32ms, 2=64ms, 3=128ms
static int  setting_driver = 0;    // 0=DirectX 11, 1=Vulkan, 2=OpenGL, 3=DirectX 12, 4=Software DIB
...
static void ApplyPersistedCoreOptions()
{
	...
	CoreSetOption("melonds_hybrid_small_screen", kNdsHybridVals[setting_nds_hybrid]);
	...
	CoreSetOption("genesis_plus_gx_cd_latency", onoff[setting_cd_latency ? 0 : 1]);
}
...
void MenuSetFullscreen(bool fs) { bool changed = (setting_fullscreen != fs); setting_fullscreen = fs; if (changed) MenuSaveSettings(); }
...
{
	if (setting_arcade_core == 1) return "cores/mame2003.dll";
	if (setting_arcade_core == 2) return "cores/mame2010.dll";
	if (setting_arcade_core == 3) return "cores/dreamcast.dll";
	return "cores/arcade_fbneo.dll";
}

static const char* GetN64CoreDll()
{
	if (setting_n64_core == 1) return "cores/n64_parallel.dll";
	if (setting_n64_core == 2) return "cores/n64_mupen.dll";
...
static std::string ResolveCoreForPath(const std::string& file_path, const std::string& dir_hint)
{
	std::string ext = fs::path(file_path).extension().string();
	std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);

	const std::string& hint = dir_hint.empty() ? file_path : dir_hint;
	auto in = [&hint](const char* needle) { return hint.find(needle) != std::string::npos; };

	// Unambiguous extensions: one system owns them.
	if (ext == ".z64" || ext == ".n64" || ext == ".v64") return GetN64CoreDll();
	if (ext == ".nes" || ext == ".fds")   return "cores/nes.dll";
	if (ext == ".sfc" || ext == ".smc")   return "cores/snes.dll";
	if (ext == ".md"  || ext == ".gen")   return "cores/genesis.dll";
	...
	{
		if (in("Arcade"))       return GetArcadeCoreDll();
		if (in("NeoGeo"))       return "cores/neogeo.dll";
		if (in("Atari5200"))    return "cores/atari5200.dll";
		if (in("Atari7800"))    return "cores/atari7800.dll";
		if (in("Atari"))        return "cores/atari2600.dll";
		...
		if (in("NES"))          return "cores/nes.dll";
		if (in("Nintendo64") || in("N64")) return GetN64CoreDll();
...
struct SystemCore { int action_id; const char* dll; };
static const SystemCore kSystemCores[] = {
	{ 101, "cores/atari2600.dll" },   { 102, "cores/genesis.dll" },
	{ 103, "cores/sms.dll" },         { 104, "cores/genesis.dll" },
	{ 105, "cores/nes.dll" },         { 106, "cores/neogeo.dll" },
	{ 107, NULL },                    { 108, "cores/psx.dll" },
	{ 109, "cores/snes.dll" },        { 110, "cores/saturn.dll" },
	{ 111, "cores/pce.dll" },         { 112, "cores/dreamcast.dll" },
	{ 113, "cores/gamecube.dll" },    { 114, "cores/nds.dll" },
	{ 115, "cores/3ds.dll" },         { 116, "cores/ps2.dll" },
	{ 117, NULL },                    { 118, "cores/gba.dll" },
	{ 119, "cores/gb.dll" },          { 120, "cores/psp.dll" },
	{ 121, "cores/dosbox_pure.dll" }, { 122, "cores/msx.dll" },
	{ 123, "cores/amiga.dll" },       { 124, "cores/c64.dll" },
	{ 125, "cores/spectrum.dll" },    { 126, "cores/32x.dll" },
	{ 127, "cores/3do.dll" },         { 128, "cores/jaguar.dll" },
	{ 129, "cores/atari5200.dll" },   { 130, "cores/atari7800.dll" },
	{ 131, "cores/coleco.dll" },      { 132, "cores/ngp.dll" },
	{ 133, "cores/wswan.dll" },       { 134, "cores/lynx.dll" },
	{ 135, "cores/pcfx.dll" },
};
...
static bool SystemHasCore(int action_id)
{
	...
	for (const auto& e : kSystemCores)
		if (e.action_id == action_id) { dll = e.dll; known = true; break; }

	if (!known) return true;               // not a system row
	if (action_id == 107) dll = GetN64CoreDll();
	if (action_id == 117) dll = GetArcadeCoreDll();
	if (!dll) return true;
...
void PopulateMainMenu()
{
	...
	if (CoreIsRunning())
	{
		...
		if (core_name == "NeoGeo" || core_name.find("Neo") != std::string::npos)
		{
			...
			const char* cd_types[] = { "CDZ", "Top-Loading", "Front-Loading", "CDZ UniBIOS" };
			...
			items.push_back({ "Memory Card", memcards[neo_memcard], false, false, 505 });
			items.push_back({ "[DIP] Settings", dip_states[neo_dip_settings], false, false, 506 });
			items.push_back({ "[DIP] Freeplay", dip_states[neo_dip_freeplay], false, false, 507 });
		}

		// 3. Display & Shaders
		items.push_back({ "Aspect Ratio", aspects[setting_aspect], false, false, 301 });
		items.push_back({ "Scanlines", FilterLabel(setting_filter), false, false, 302 });
		...
		if (core_name.find("Master System") != std::string::npos)
		{
			...
			items.push_back({ "FM Audio (YM2413)", fm[setting_sms_fm], false, false, 509 });
		}
		...
		{
			items.push_back({ "Layout Telas", kNdsLayoutNames[setting_nds_layout], false, false, 510 });
			items.push_back({ "Espaco Telas", kNdsGapVals[setting_nds_gap], false, false, 511 });
			if (setting_nds_layout >= 6)
				items.push_back({ "Tela Pequena", kNdsHybridNames[setting_nds_hybrid], false, false, 512 });
		}
		...
		if (core_name.find("Genesis") != std::string::npos && CoreIsDiscGame())
		{
			...
			items.push_back({ "Cache do CD", od[setting_cd_precache], false, false, 513 });
			items.push_back({ "Acesso do CD", acc[setting_cd_latency], false, false, 514 });
		}
		...
		if (RaIsEnabled())
		{
			...
			{
				snprintf(ra_val, sizeof(ra_val), "%d/%d%s",
					RaGetAchievementsUnlocked(), RaGetAchievementCount(),
					RaHasPendingUnlocks() ? " !" : "");
			}
			else
			{
	...
	else
	{
		...
		if (SystemHasCore(117)) items.push_back({ "Arcade", ">", true, true, 117 });
		if (SystemHasCore(101)) items.push_back({ "Atari 2600", ">", true, true, 101 });
		if (SystemHasCore(129)) items.push_back({ "Atari 5200", ">", true, true, 129 });
		if (SystemHasCore(130)) items.push_back({ "Atari 7800", ">", true, true, 130 });
		if (SystemHasCore(128)) items.push_back({ "Atari Jaguar", ">", true, true, 128 });
		if (SystemHasCore(134)) items.push_back({ "Atari Lynx", ">", true, true, 134 });
		if (SystemHasCore(131)) items.push_back({ "ColecoVision", ">", true, true, 131 });
		if (SystemHasCore(124)) items.push_back({ "Commodore 64", ">", true, true, 124 });
		if (SystemHasCore(123)) items.push_back({ "Commodore Amiga", ">", true, true, 123 });
		if (SystemHasCore(112)) items.push_back({ "Dreamcast", ">", true, true, 112 });
		if (SystemHasCore(119)) items.push_back({ "Game Boy / Color", ">", true, true, 119 });
		if (SystemHasCore(118)) items.push_back({ "Game Boy Advance", ">", true, true, 118 });
		if (SystemHasCore(113)) items.push_back({ "GameCube", ">", true, true, 113 });
		if (SystemHasCore(102)) items.push_back({ "Genesis / Mega Drive", ">", true, true, 102 });
		if (SystemHasCore(103)) items.push_back({ "Master System", ">", true, true, 103 });
		if (SystemHasCore(104)) items.push_back({ "Mega CD", ">", true, true, 104 });
		if (SystemHasCore(121)) items.push_back({ "MS-DOS", ">", true, true, 121 });
		if (SystemHasCore(122)) items.push_back({ "MSX / MSX2", ">", true, true, 122 });
		if (SystemHasCore(105)) items.push_back({ "NES / Famicom", ">", true, true, 105 });
		if (SystemHasCore(132)) items.push_back({ "Neo Geo Pocket", ">", true, true, 132 });
		if (SystemHasCore(106)) items.push_back({ "NeoGeo / CD", ">", true, true, 106 });
		if (SystemHasCore(115)) items.push_back({ "Nintendo 3DS", ">", true, true, 115 });
		if (SystemHasCore(107)) items.push_back({ "Nintendo 64", ">", true, true, 107 });
		if (SystemHasCore(114)) items.push_back({ "Nintendo DS", ">", true, true, 114 });
		if (SystemHasCore(127)) items.push_back({ "Panasonic 3DO", ">", true, true, 127 });
		if (SystemHasCore(135)) items.push_back({ "PC-FX", ">", true, true, 135 });
		if (SystemHasCore(108)) items.push_back({ "PlayStation 1", ">", true, true, 108 });
		if (SystemHasCore(116)) items.push_back({ "PlayStation 2 (PS2)", ">", true, true, 116 });
		if (SystemHasCore(120)) items.push_back({ "PlayStation Portable (PSP)", ">", true, true, 120 });
		if (SystemHasCore(110)) items.push_back({ "Saturn", ">", true, true, 110 });
		if (SystemHasCore(126)) items.push_back({ "Sega 32X", ">", true, true, 126 });
		if (SystemHasCore(109)) items.push_back({ "SNES / Super Famicom", ">", true, true, 109 });
		if (SystemHasCore(111)) items.push_back({ "TurboGrafx 16 / PCE", ">", true, true, 111 });
		if (SystemHasCore(133)) items.push_back({ "WonderSwan", ">", true, true, 133 });
		if (SystemHasCore(125)) items.push_back({ "ZX Spectrum", ">", true, true, 125 });
		...
		for (const auto& it : items)
			if (it.action_id >= 101 && it.action_id <= 135) g_system_count++;
	}
...
void PopulateControllerSettings()
{
	...
	{
		XINPUT_STATE st;
		if (XInputGetState(i, &st) == ERROR_SUCCESS) pads++;
	}

	char pad_str[24];
	snprintf(pad_str, sizeof(pad_str), "%d conectado%s", pads, pads == 1 ? "" : "s");
	items.push_back({ "Gamepads", pad_str, false, false, 0 });

	const char* dev_names[] = { "Gamepad", "Teclado" };
	items.push_back({ "Dispositivo", dev_names[setting_pad_device], false, false, 500 });
	...
	{
		const char* val = (capture_bind == i) ? "<pressione>" :
			(setting_pad_device == 0 ? InputBindPadName(i) : InputBindKeyName(i));
		items.push_back({ InputBindLabel(i), val, false, true, 700 + i });
	}

	items.push_back({ "Restaurar", "Padrao", false, true, 799 });

	if (capture_bind < 0 && selected_idx < 1) selected_idx = 1;
	scroll_top = 0;
}
...
void PopulateVideoSettings()
{
	...
	const char* aspects[] = { "Original", "4:3", "16:9" };
	const char* wallpapers[] = { "None", "Static Noise", "Parallax Stars", "Cyber Grid", "Flavor Chef", "Flavor Arcade" };
	const char* displays[] = { "Windowed", "Fullscreen" };
	const char* themes[] = { "Red", "Blue", "Green", "Amber", "Gray", "Dark" };
	const char* drivers[] = { "DirectX 11", "Vulkan", "OpenGL", "DirectX 12", "Software DIB" };

	items.push_back({ "Aspect Ratio", aspects[setting_aspect], false, false, 301 });
	items.push_back({ "CRT Shader", FilterLabel(setting_filter), false, false, 302 });
	items.push_back({ "Wallpaper", wallpapers[setting_wallpaper], false, false, 303 });
	items.push_back({ "Display", displays[setting_fullscreen ? 1 : 0], false, false, 304 });
	...
	items.push_back({ "3D Acceleration", hw3d[setting_hw_render], false, false, 310 });
	...
	const char* arcade_cores[] = { "FinalBurn Neo", "MAME 2003", "MAME 2010", "Flycast Naomi" };
	items.push_back({ "Arcade Core", arcade_cores[setting_arcade_core], false, false, 311 });

	char osd_to[16];
	if (kOsdTimeouts[setting_osd_timeout] == 0) snprintf(osd_to, sizeof(osd_to), "Nunca");
	else snprintf(osd_to, sizeof(osd_to), "%ds", kOsdTimeouts[setting_osd_timeout]);
...
void PopulateNetplay()
{
	...
	if (NetplayGetState() == NETPLAY_LISTENING) role_str = "Hosting...";
	else if (NetplayGetState() == NETPLAY_CONNECTING) role_str = "Connecting...";
	else if (NetplayGetState() == NETPLAY_CONNECTED)
	{
		role_str = (NetplayGetRole() == NETPLAY_HOST) ? "Connected (Host)" : "Connected (Client)";
	}
...
void PopulateRetroAchievements()
{
	...
	{
		items.push_back({ "Usuario", RaIsLoggedIn() ? RaGetUserName() : "-", false, false, 0 });
		items.push_back({ "Modo", RaIsHardcoreActive() ? "Hardcore" : "Softcore", false, false, 0 });

		if (RaGetAchievementCount() > 0)
		{
			char v[40];
			snprintf(v, sizeof(v), "%d/%d", RaGetAchievementsUnlocked(), RaGetAchievementCount());
...
void PopulateAbout()
{
	...
	if (core_files > 0 && core_files != APP_CORE_FILES)
	{
		...
		if (lf)
		{
			fprintf(lf, "[WARN] [ABOUT] cores/ tem %d DLLs, mas APP_CORE_FILES diz %d - atualize app_info.h e o site\n", core_files, APP_CORE_FILES);
			fclose(lf);
		}
	}
	...
	items.push_back({ "Conquistas", RaIsEnabled() ? "Ativado" : "Desativado", false, false, 0 });
...
void PopulateBrowse(const std::string& dirpath)
{
	...
	if (dirpath != "roms" && dirpath != "." && dirpath != "/" && dirpath != "\\")
	{
	...
	try
	{
		...
		std::sort(dirs.begin(), dirs.end(), [](const MenuItem& a, const MenuItem& b) { return a.label < b.label; });
		std::sort(files.begin(), files.end(), [](const MenuItem& a, const MenuItem& b) { return a.label < b.label; });

		for (const auto& d : dirs) items.push_back(d);
		for (const auto& f : files) items.push_back(f);
	}
	catch (...)
	{
...
static void BrowseGoUp()
{
	...
	if (parent.empty() || parent == "roms" || parent == ".")
	{
		...
		if (selected_idx < 0 || selected_idx >= (int)items.size()) selected_idx = 0;
		return;
	}
...
static void MenuLoadSettings()
{
	...
	if (!f) f = fopen(LEGACY_SETTINGS_PATH, "rb");
	if (!f)
	{
	...
	while (fgets(line, sizeof(line), f))
	{
		...
		*eq = '\0';
		...
		if (!strcmp(key, "aspect"))              setting_aspect = ClampInt(iv, 0, 2);
		else if (!strcmp(key, "filter"))         setting_filter = MigrateFilter(ClampInt(iv, 0, 9));
		else if (!strcmp(key, "wallpaper"))      setting_wallpaper = ClampInt(iv, 0, 5);
		else if (!strcmp(key, "fullscreen"))     setting_fullscreen = (iv != 0);
		else if (!strcmp(key, "theme"))          setting_theme = ClampInt(iv, 0, 5);
		else if (!strcmp(key, "deadzone"))       setting_deadzone = ClampInt(iv, 0, 3);
		else if (!strcmp(key, "pad_device"))     setting_pad_device = ClampInt(iv, 0, 1);
		...
		else if (!strcmp(key, "latency"))        setting_latency = ClampInt(iv, 0, 3);
		else if (!strcmp(key, "driver"))         setting_driver = ClampInt(iv, 0, 4);
		else if (!strcmp(key, "sync"))           setting_sync = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "vsync"))          setting_vsync = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "n64_core"))       setting_n64_core = ClampInt(iv, 0, 2);
		else if (!strcmp(key, "arcade_core"))    setting_arcade_core = ClampInt(iv, 0, 3);
		else if (!strcmp(key, "osd_timeout"))    setting_osd_timeout = ClampInt(iv, 0, kOsdTimeoutCount - 1);
		else if (!strcmp(key, "hw_render"))      setting_hw_render = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "sms_fm"))         setting_sms_fm = ClampInt(iv, 0, 2);
		else if (!strcmp(key, "nds_layout"))   setting_nds_layout = ClampInt(iv, 0, kNdsLayoutCount - 1);
		else if (!strcmp(key, "nds_gap"))      setting_nds_gap = ClampInt(iv, 0, kNdsGapCount - 1);
		else if (!strcmp(key, "nds_hybrid"))   setting_nds_hybrid = ClampInt(iv, 0, kNdsHybridCount - 1);
		else if (!strcmp(key, "cd_precache"))  setting_cd_precache = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "cd_latency"))   setting_cd_latency = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "volume"))         CoreSetVolume(ClampInt(iv, 0, 100));
		else if (!strcmp(key, "mute"))           CoreSetMute(iv != 0);
		else if (!strcmp(key, "neo_sys"))        neo_sys = ClampInt(iv, 0, 2);
		else if (!strcmp(key, "neo_bios"))       neo_bios = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "neo_cd_type"))    neo_cd_type = ClampInt(iv, 0, 3);
		else if (!strcmp(key, "neo_cd_region"))  neo_cd_region = ClampInt(iv, 0, 3);
		else if (!strcmp(key, "neo_memcard"))    neo_memcard = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "neo_dip_settings")) neo_dip_settings = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "neo_dip_freeplay")) neo_dip_freeplay = ClampInt(iv, 0, 1);
		else if (!strcmp(key, "netplay_ip") && val[0]) join_ip_input = val;
	}
	fclose(f);
}
...
void MenuProcessKey(MenuKey key)
{
	...
	if (SettingsSnapshot() != before) MenuSaveSettings();
}
...
void MenuRun()
{
	...
	if (OsdIsEnabled() && CoreIsRunning() && capture_bind < 0)
	{
		...
		if (secs > 0)
		{
			if (g_last_input_tick == 0) g_last_input_tick = GetTickCount();
	...
	if (capture_bind >= 0)
	{
		// Arm only once every key and button is up, otherwise the same press that opened
		// the capture binds itself instantly.
		if (!capture_armed)
		{
		...
		else
		{
			...
			else if (vk != 0)
			{
				...
				PopulateControllerSettings();
				selected_idx = kBindRow0 + target;
				if (ok) MenuSaveSettings();
				else CoreSetToast("ESSA TECLA E RESERVADA PARA O MENU", 120);
			}
		}
	}
	...
	OsdClear();
	OsdSetTitle(current_title.c_str(), (current_state != STATE_MAIN) ? OSD_ARROW_LEFT : 0);
...
static void MenuProcessKeyImpl(MenuKey key)
{
	...
	if (capture_bind >= 0) return;

	if (key == KEY_MENU_TOGGLE)
	{
		if (OsdIsEnabled())
		{
			if (CoreIsRunning()) OsdDisable();
		}
		else
		{
			OsdEnable(0);
	...
	case KEY_RIGHT:
	{
		...
		if (item.action_id == 301) // Aspect
		{
			...
			if (CoreIsRunning() && current_state == STATE_MAIN) PopulateMainMenu();
			else PopulateVideoSettings();
			selected_idx = cur;
		}
		else if (item.action_id == 302) // CRT Shaders
		{
			int cur = selected_idx;
			setting_filter = kFilters[(FilterIndex(setting_filter) + delta + kFilterCount) % kFilterCount].mode;
			char msg[64];
		...
		else if (item.action_id == 509) // Master System FM
		{
			...
			char msg[64];
			snprintf(msg, sizeof(msg), "FM AUDIO: %s (recarregue o jogo)", names[setting_sms_fm]);
		...
		{
			int cur = selected_idx;
			setting_nds_layout = (setting_nds_layout + delta + kNdsLayoutCount) % kNdsLayoutCount;
			CoreSetOption("melonds_screen_layout", kNdsLayoutVals[setting_nds_layout]);
			char msg[64];
			snprintf(msg, sizeof(msg), "LAYOUT: %s", kNdsLayoutNames[setting_nds_layout]);
			CoreSetToast(msg, 120);
			PopulateMainMenu();
			selected_idx = cur;
		}
		...
		{
			int cur = selected_idx;
			setting_nds_hybrid = (setting_nds_hybrid + delta + kNdsHybridCount) % kNdsHybridCount;
			CoreSetOption("melonds_hybrid_small_screen", kNdsHybridVals[setting_nds_hybrid]);
			char msg[64];
			snprintf(msg, sizeof(msg), "TELA PEQUENA: %s", kNdsHybridNames[setting_nds_hybrid]);
			CoreSetToast(msg, 120);
			PopulateMainMenu();
			selected_idx = cur;
		}
		else if (item.action_id == 513) // Mega CD image cache
		{
			...
			CoreSetOption("genesis_plus_gx_cd_precache", setting_cd_precache ? "enabled" : "disabled");
			CoreSetToast(setting_cd_precache ? "CACHE DO CD: LIGADO (recarregue o jogo)"
			                                 : "CACHE DO CD: DESLIGADO", 180);
			PopulateMainMenu();
			selected_idx = cur;
		}
		else if (item.action_id == 514) // Mega CD access time
		{
			...
			CoreSetOption("genesis_plus_gx_cd_latency", setting_cd_latency ? "disabled" : "enabled");
			// The core warns a few games crash when CD data arrives too early,
			// so this is offered, not defaulted.
			CoreSetToast(setting_cd_latency ? "ACESSO DO CD: RAPIDO (pode falhar em alguns jogos)"
			                                : "ACESSO DO CD: REAL", 200);
			PopulateMainMenu();
			selected_idx = cur;
		}
		else if (item.action_id == 310) // 3D acceleration
		{
			...
			CoreSetToast(setting_hw_render == 1
				? "3D: OPENGL (RECARREGUE O JOGO)"
				: "3D: SOFTWARE (RECARREGUE O JOGO)", 200);
			PopulateVideoSettings();
			selected_idx = 6;
		}
		else if (item.action_id == 309) // N64 core
		{
			...
			const char* names[] = { "DEFAULT (NO ACHIEVEMENTS)", "PARALLEL-N64", "MUPEN64PLUS" };
			char msg[80];
		...
		{
			setting_osd_timeout = (setting_osd_timeout + delta + kOsdTimeoutCount) % kOsdTimeoutCount;
			char msg[64];
			if (kOsdTimeouts[setting_osd_timeout] == 0)
				snprintf(msg, sizeof(msg), "MENU: NAO OCULTA SOZINHO");
			else
				snprintf(msg, sizeof(msg), "MENU OCULTA APOS %ds SEM USO", kOsdTimeouts[setting_osd_timeout]);
			CoreSetToast(msg, 150);
			PopulateVideoSettings();
			selected_idx = 8;
		}
		else if (item.action_id == 311) // Arcade core
		{
			...
			const char* names[] = { "FINALBURN NEO", "MAME 2003 (romset 0.78)", "MAME 2010 (romset 0.139)", "FLYCAST (NAOMI / ATOMISWAVE)" };
			char msg[80];
		...
		else if (item.action_id == 308) // V-Sync
		{
			...
			CoreSetToast(setting_vsync == 1 ? "V-SYNC ENABLED" : "V-SYNC DISABLED", 120);
			PopulateVideoSettings();
			selected_idx = 6;
		}
	...
	case KEY_SELECT:
	{
		...
		else if (current_state == STATE_MAIN)
		{
			...
			{
				main_menu_saved_idx = selected_idx;
				if (item.action_id == 101) { current_state = STATE_BROWSE; PopulateBrowse("roms/Atari2600"); }
				else if (item.action_id == 102) { current_state = STATE_BROWSE; PopulateBrowse("roms/Genesis"); }
				else if (item.action_id == 103) { current_state = STATE_BROWSE; PopulateBrowse("roms/MasterSystem"); }
				else if (item.action_id == 104) { current_state = STATE_BROWSE; PopulateBrowse("roms/MegaCD"); }
				else if (item.action_id == 105) { current_state = STATE_BROWSE; PopulateBrowse("roms/NES"); }
				else if (item.action_id == 106) { current_state = STATE_BROWSE; PopulateBrowse("roms/NeoGeo"); }
				else if (item.action_id == 107) { current_state = STATE_BROWSE; PopulateBrowse("roms/Nintendo64"); }
				else if (item.action_id == 108) { current_state = STATE_BROWSE; PopulateBrowse("roms/PlayStation"); }
				else if (item.action_id == 109) { current_state = STATE_BROWSE; PopulateBrowse("roms/SNES"); }
				else if (item.action_id == 110) { current_state = STATE_BROWSE; PopulateBrowse("roms/Saturn"); }
				else if (item.action_id == 111) { current_state = STATE_BROWSE; PopulateBrowse("roms/TurboGrafx16"); }
				else if (item.action_id == 112) { current_state = STATE_BROWSE; PopulateBrowse("roms/Dreamcast"); }
				else if (item.action_id == 113) { current_state = STATE_BROWSE; PopulateBrowse("roms/GameCube"); }
				else if (item.action_id == 114) { current_state = STATE_BROWSE; PopulateBrowse("roms/NDS"); }
				else if (item.action_id == 115) { current_state = STATE_BROWSE; PopulateBrowse("roms/3DS"); }
				else if (item.action_id == 116) { current_state = STATE_BROWSE; PopulateBrowse("roms/PlayStation2"); }
				else if (item.action_id == 117) { current_state = STATE_BROWSE; PopulateBrowse("roms/Arcade"); }
				else if (item.action_id == 118) { current_state = STATE_BROWSE; PopulateBrowse("roms/GBA"); }
				else if (item.action_id == 119) { current_state = STATE_BROWSE; PopulateBrowse("roms/GameBoy"); }
				else if (item.action_id == 120) { current_state = STATE_BROWSE; PopulateBrowse("roms/PSP"); }
				else if (item.action_id == 121) { current_state = STATE_BROWSE; PopulateBrowse("roms/DOS"); }
				else if (item.action_id == 122) { current_state = STATE_BROWSE; PopulateBrowse("roms/MSX"); }
				else if (item.action_id == 123) { current_state = STATE_BROWSE; PopulateBrowse("roms/Amiga"); }
				else if (item.action_id == 124) { current_state = STATE_BROWSE; PopulateBrowse("roms/C64"); }
				else if (item.action_id == 125) { current_state = STATE_BROWSE; PopulateBrowse("roms/ZXSpectrum"); }
				else if (item.action_id == 126) { current_state = STATE_BROWSE; PopulateBrowse("roms/32X"); }
				else if (item.action_id == 127) { current_state = STATE_BROWSE; PopulateBrowse("roms/3DO"); }
				else if (item.action_id == 128) { current_state = STATE_BROWSE; PopulateBrowse("roms/Jaguar"); }
				else if (item.action_id == 129) { current_state = STATE_BROWSE; PopulateBrowse("roms/Atari5200"); }
				else if (item.action_id == 130) { current_state = STATE_BROWSE; PopulateBrowse("roms/Atari7800"); }
				else if (item.action_id == 131) { current_state = STATE_BROWSE; PopulateBrowse("roms/ColecoVision"); }
				else if (item.action_id == 132) { current_state = STATE_BROWSE; PopulateBrowse("roms/NGP"); }
				else if (item.action_id == 133) { current_state = STATE_BROWSE; PopulateBrowse("roms/WonderSwan"); }
				else if (item.action_id == 134) { current_state = STATE_BROWSE; PopulateBrowse("roms/Lynx"); }
				else if (item.action_id == 135) { current_state = STATE_BROWSE; PopulateBrowse("roms/PCFX"); }
			}
		...
		{
			if (item.action_id == 201) { current_state = STATE_VIDEO; PopulateVideoSettings(); }
			else if (item.action_id == 202) { current_state = STATE_AUDIO; PopulateAudioSettings(); }
			else if (item.action_id == 203) { current_state = STATE_CONTROLLER; PopulateControllerSettings(); }
			else if (item.action_id == 204) { current_state = STATE_NETPLAY; PopulateNetplay(); }
			else if (item.action_id == 206) { current_state = STATE_ABOUT; PopulateRetroAchievements(); }
			else if (item.action_id == 205) { current_state = STATE_ABOUT; PopulateAbout(); }
		}
		...
		else if (current_state == STATE_BROWSE)
		{
			...
			{
				std::string full_rom_path = (fs::path(current_dir) / item.label).string();
	...
	case KEY_CANCEL:
		...
		else if (current_state == STATE_VIDEO || current_state == STATE_AUDIO || current_state == STATE_CONTROLLER || current_state == STATE_NETPLAY || current_state == STATE_ABOUT)
		{
			current_state = STATE_SETTINGS;
			PopulateSettings();
		}
...
bool MenuLaunchGamePath(const std::string& full_rom_path)
{
	...
	if (core_dll.empty()) return false;
