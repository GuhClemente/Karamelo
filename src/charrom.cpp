...
// charrom.cpp with a table sourced from Unscii (https://github.com/viznut/
// unscii), whose author explicitly places it in the Public Domain / CC0
// (see unscii's own README: "You can consider it Public Domain (or CC-0)").
//
// This codebase stores each glyph column-major: charfont[ch][col] is one
// byte per column (0-7, left to right), and bit `row` of that byte is the
// pixel at that column/row (confirmed against DrawCharTo() and the OSD
// raster reader in main_win32.cpp, and against the original ported array's
// own 'A' entry). Unscii's own .hex format is row-major with bit 7 as the
// leftmost pixel, so every glyph below is transposed from Unscii's data,
// not copied byte-for-byte.
//
// Glyphs 32-126 come from unscii-8.hex's Basic Latin range. Every other
// index this codebase actually reads - confirmed via grep across all of
// src/, not just osd.cpp - is a plain geometric triangle/chevron authored
// directly in this column-major convention, not derived from Unscii,
// MiSTer, or anything else:
//   0x10/0x11/0x14/0x15  the paging-arrow cell OsdWriteOffset draws itself
//   0x16                 menu.cpp's own "opens a submenu" row marker,
//                         embedded as a plain byte in ordinary OSD strings
// Every remaining index (the bulk of the MiSTer-specific icon set - Atari
// logo, bluetooth glyph, memory-fill icons, etc. - none of which this app's
// own code ever indexes into) is left blank rather than invented busywork.
//
// MiSTer's own LoadFont()/tempfont (loading a replacement 8x8 font from a
// user-supplied file at runtime) is not ported either: grepping this app's
// own src/ turns up no caller anywhere, so there was nothing to keep it
// for.
unsigned char charfont[256][8] =
{
	{ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 },         // 0    [0x0]
	{ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 },         // 1    [0x1]
	{ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 },         // 2    [0x2]
	{ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 },         // 3    [0x3]
	...
	{ 0x18,0x18,0x3C,0x3C,0x7E,0x7E,0xFF,0xFF },         // 16   [0x10]
	{ 0xFF,0xFF,0x7E,0x7E,0x3C,0x3C,0x18,0x18 },         // 17   [0x11]
	{ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 },         // 18   [0x12]
	{ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 },         // 19   [0x13]
	{ 0x08,0x04,0x02,0x01,0x01,0x02,0x04,0x08 },         // 20   [0x14]
	{ 0x10,0x20,0x40,0x80,0x80,0x40,0x20,0x10 },         // 21   [0x15]
	{ 0xFF,0xFF,0x7E,0x7E,0x3C,0x3C,0x18,0x18 },         // 22   [0x16]
	...
	{ 0x00,0x07,0x07,0x00,0x00,0x07,0x07,0x00 },         // 34   [0x22] '"'
	...
	{ 0x00,0x3E,0x7F,0x49,0x45,0x7F,0x3E,0x00 },         // 48   [0x30] '0'
	{ 0x00,0x40,0x42,0x7F,0x7F,0x40,0x40,0x00 },         // 49   [0x31] '1'
	{ 0x00,0x62,0x73,0x59,0x4D,0x47,0x42,0x00 },         // 50   [0x32] '2'
	{ 0x00,0x22,0x63,0x49,0x49,0x7F,0x36,0x00 },         // 51   [0x33] '3'
	{ 0x18,0x1C,0x16,0x13,0x7F,0x7F,0x10,0x00 },         // 52   [0x34] '4'
	{ 0x00,0x27,0x67,0x45,0x45,0x7D,0x39,0x00 },         // 53   [0x35] '5'
	{ 0x00,0x3C,0x7E,0x4B,0x49,0x79,0x30,0x00 },         // 54   [0x36] '6'
	{ 0x00,0x01,0x01,0x71,0x79,0x0F,0x07,0x00 },         // 55   [0x37] '7'
	{ 0x00,0x36,0x7F,0x49,0x49,0x7F,0x36,0x00 },         // 56   [0x38] '8'
	{ 0x00,0x06,0x4F,0x49,0x69,0x3F,0x1E,0x00 },         // 57   [0x39] '9'
	...
	{ 0x3E,0x7F,0x41,0x5D,0x5D,0x5F,0x1E,0x00 },         // 64   [0x40] '@'
	{ 0x00,0x7C,0x7E,0x13,0x13,0x7E,0x7C,0x00 },         // 65   [0x41] 'A'
	{ 0x00,0x7F,0x7F,0x49,0x49,0x7F,0x36,0x00 },         // 66   [0x42] 'B'
	{ 0x00,0x3E,0x7F,0x41,0x41,0x63,0x22,0x00 },         // 67   [0x43] 'C'
	{ 0x00,0x7F,0x7F,0x41,0x63,0x3E,0x1C,0x00 },         // 68   [0x44] 'D'
	{ 0x00,0x7F,0x7F,0x49,0x49,0x49,0x41,0x00 },         // 69   [0x45] 'E'
	{ 0x00,0x7F,0x7F,0x09,0x09,0x09,0x01,0x00 },         // 70   [0x46] 'F'
	{ 0x00,0x3E,0x7F,0x41,0x49,0x7B,0x7A,0x00 },         // 71   [0x47] 'G'
	{ 0x00,0x7F,0x7F,0x08,0x08,0x7F,0x7F,0x00 },         // 72   [0x48] 'H'
	{ 0x00,0x41,0x41,0x7F,0x7F,0x41,0x41,0x00 },         // 73   [0x49] 'I'
	{ 0x00,0x20,0x60,0x40,0x40,0x7F,0x3F,0x00 },         // 74   [0x4a] 'J'
	{ 0x7F,0x7F,0x08,0x1C,0x36,0x63,0x41,0x00 },         // 75   [0x4b] 'K'
	{ 0x00,0x7F,0x7F,0x40,0x40,0x40,0x40,0x00 },         // 76   [0x4c] 'L'
	{ 0x7F,0x7F,0x06,0x0C,0x06,0x7F,0x7F,0x00 },         // 77   [0x4d] 'M'
	{ 0x7F,0x7F,0x06,0x0C,0x18,0x7F,0x7F,0x00 },         // 78   [0x4e] 'N'
	{ 0x00,0x3E,0x7F,0x41,0x41,0x7F,0x3E,0x00 },         // 79   [0x4f] 'O'
	{ 0x00,0x7F,0x7F,0x09,0x09,0x0F,0x06,0x00 },         // 80   [0x50] 'P'
	{ 0x00,0x3E,0x7F,0x41,0x21,0x7F,0x5E,0x00 },         // 81   [0x51] 'Q'
	{ 0x00,0x7F,0x7F,0x09,0x19,0x7F,0x66,0x00 },         // 82   [0x52] 'R'
	{ 0x00,0x26,0x6F,0x49,0x49,0x7B,0x32,0x00 },         // 83   [0x53] 'S'
	{ 0x00,0x01,0x01,0x7F,0x7F,0x01,0x01,0x00 },         // 84   [0x54] 'T'
	{ 0x00,0x3F,0x7F,0x40,0x40,0x7F,0x3F,0x00 },         // 85   [0x55] 'U'
	{ 0x00,0x1F,0x3F,0x60,0x60,0x3F,0x1F,0x00 },         // 86   [0x56] 'V'
	{ 0x7F,0x7F,0x30,0x18,0x30,0x7F,0x7F,0x00 },         // 87   [0x57] 'W'
	{ 0x41,0x63,0x36,0x1C,0x1C,0x36,0x63,0x41 },         // 88   [0x58] 'X'
	{ 0x01,0x03,0x06,0x7C,0x7C,0x06,0x03,0x01 },         // 89   [0x59] 'Y'
	{ 0x00,0x61,0x71,0x59,0x4D,0x47,0x43,0x00 },         // 90   [0x5a] 'Z'
	...
	{ 0x00,0x00,0x00,0x01,0x03,0x06,0x04,0x00 },         // 96   [0x60] '`'
	{ 0x00,0x20,0x74,0x54,0x54,0x7C,0x78,0x00 },         // 97   [0x61] 'a'
	{ 0x00,0x7F,0x7F,0x44,0x44,0x7C,0x38,0x00 },         // 98   [0x62] 'b'
	{ 0x00,0x38,0x7C,0x44,0x44,0x44,0x00,0x00 },         // 99   [0x63] 'c'
	{ 0x00,0x38,0x7C,0x44,0x44,0x7F,0x7F,0x00 },         // 100  [0x64] 'd'
	{ 0x00,0x38,0x7C,0x54,0x54,0x5C,0x18,0x00 },         // 101  [0x65] 'e'
	{ 0x00,0x04,0x7E,0x7F,0x05,0x05,0x00,0x00 },         // 102  [0x66] 'f'
	{ 0x00,0x98,0xBC,0xA4,0xA4,0xFC,0x7C,0x00 },         // 103  [0x67] 'g'
	{ 0x00,0x7F,0x7F,0x04,0x04,0x7C,0x78,0x00 },         // 104  [0x68] 'h'
	{ 0x00,0x00,0x04,0x7D,0x7D,0x40,0x40,0x00 },         // 105  [0x69] 'i'
	{ 0x00,0x80,0x80,0x80,0xFD,0x7D,0x00,0x00 },         // 106  [0x6a] 'j'
	{ 0x00,0x7F,0x7F,0x10,0x38,0x6C,0x44,0x00 },         // 107  [0x6b] 'k'
	{ 0x00,0x00,0x01,0x7F,0x7F,0x40,0x40,0x00 },         // 108  [0x6c] 'l'
	{ 0x7C,0x7C,0x08,0x38,0x0C,0x7C,0x78,0x00 },         // 109  [0x6d] 'm'
	{ 0x00,0x7C,0x7C,0x04,0x04,0x7C,0x78,0x00 },         // 110  [0x6e] 'n'
	{ 0x00,0x38,0x7C,0x44,0x44,0x7C,0x38,0x00 },         // 111  [0x6f] 'o'
	{ 0x00,0xFC,0xFC,0x24,0x24,0x3C,0x18,0x00 },         // 112  [0x70] 'p'
	{ 0x00,0x18,0x3C,0x24,0x24,0xFC,0xFC,0x00 },         // 113  [0x71] 'q'
	{ 0x00,0x7C,0x7C,0x04,0x04,0x0C,0x08,0x00 },         // 114  [0x72] 'r'
	{ 0x00,0x48,0x5C,0x54,0x54,0x74,0x24,0x00 },         // 115  [0x73] 's'
	{ 0x00,0x04,0x3F,0x7F,0x44,0x44,0x44,0x00 },         // 116  [0x74] 't'
	{ 0x00,0x3C,0x7C,0x40,0x40,0x7C,0x7C,0x00 },         // 117  [0x75] 'u'
	{ 0x00,0x1C,0x3C,0x60,0x60,0x3C,0x1C,0x00 },         // 118  [0x76] 'v'
	{ 0x1C,0x7C,0x60,0x30,0x60,0x7C,0x1C,0x00 },         // 119  [0x77] 'w'
	{ 0x44,0x6C,0x38,0x10,0x38,0x6C,0x44,0x00 },         // 120  [0x78] 'x'
	{ 0x00,0x1C,0xBC,0xA0,0xA0,0xFC,0x7C,0x00 },         // 121  [0x79] 'y'
	{ 0x00,0x44,0x64,0x74,0x5C,0x4C,0x44,0x00 },         // 122  [0x7a] 'z'
