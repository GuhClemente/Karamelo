...
// Upstream's unity build omits the AV codec, but libchdr_chd.c references it
// unconditionally, so the link fails without it. It only matters for laserdisc
// CHDs, never for the CD images we hash - it is here purely to resolve.
#include "src/libchdr_codec_avhuff.c"
