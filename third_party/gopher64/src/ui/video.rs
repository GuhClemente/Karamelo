...
pub static LIBRETRO_MODE: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);
pub static VIDEO_REFRESH_CB: std::sync::Mutex<Option<unsafe extern "C" fn(*const std::ffi::c_void, u32, u32, usize)>> =
...
pub fn init(device: &mut device::Device, netplay: bool) {
    ...
    if LIBRETRO_MODE.load(std::sync::atomic::Ordering::Relaxed) {
        flags = sdl3_sys::video::SDL_WINDOW_VULKAN | sdl3_sys::video::SDL_WINDOW_HIDDEN;
    } else if device.ui.video.fullscreen {
...
unsafe extern "C" {
    pub fn rdp_get_framebuffer(
        out_pixels: *mut *const u32,
        out_width: *mut u32,
        out_height: *mut u32,
    ) -> bool;
}
...
pub fn render_frame() {
    if LIBRETRO_MODE.load(std::sync::atomic::Ordering::Relaxed) {
        let cb_opt = *VIDEO_REFRESH_CB.lock().unwrap();
        if let Some(cb) = cb_opt {
            let mut pixels_ptr: *const u32 = std::ptr::null();
            let mut width: u32 = 0;
            let mut height: u32 = 0;
            if unsafe { rdp_get_framebuffer(&mut pixels_ptr, &mut width, &mut height) }
                && !pixels_ptr.is_null()
                && width > 0
                && height > 0
            {
                let count = (width * height) as usize;
                let src_slice = unsafe { std::slice::from_raw_parts(pixels_ptr, count) };
