...
1. **Fontes Únicas da Verdade (Não Invente Números)**:
   ...
   - Contagem e Compatibilidade de Ports: Leia SEMPRE de `CREDITS.md` e verifique em `src/port_runner.cpp` (55 ports totais: 55 Windows 🪟, 32 Linux 🐧, 24 macOS 🍎).
...
- **Contagem total**: 55 ports catalogados (55 Windows 🪟, 32 Linux 🐧, 24 macOS 🍎).
...
- **Regras de Execução no macOS**:
  ...
  - Flag de inspeção: `./app/Karamelo --list-ports` lista os ports disponíveis para o SO em que o binário foi compilado (24 no macOS, 32 no Linux, 55 no Windows).
