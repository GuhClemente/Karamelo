#!/bin/bash
# =====================================================================
#   Karamelo - Linux Release Packaging Pipeline
# =====================================================================
set -euo pipefail
...
echo "======================================================="
echo "  Karamelo - Linux Packaging & Release Generator"
echo "======================================================="
echo ""
...
DIST_NAME="Karamelo_v${APP_VER}_Linux64"
...
echo "[1/5] Compilando executavel nativo Linux..."
./compile_linux.sh
...
# 3. Garantir que os cores Linux existem
echo ""
echo "[2/5] Verificando cores libretro Linux (*.so)..."
missing_cores=0
for check_core in cores/genesis.so cores/snes.so cores/nes.so cores/n64.so cores/psx.so; do
...
if [ "$missing_cores" -eq 1 ]; then
    echo "  Alguns cores estao faltando. Executando download_cores_linux.sh..."
...
# Copiar cores Linux (.so)
if ls app/cores/*.so 1>/dev/null 2>&1; then
    cp -a app/cores/*.so "$DIST_DIR/cores/"
elif ls cores/*.so 1>/dev/null 2>&1; then
    cp -a cores/*.so "$DIST_DIR/cores/"
fi
...
echo ""
echo "[5/5] Atualizando dist/Karamelo_linux e dist/version.json..."
cp -a "app/Karamelo" "dist/Karamelo_linux"
chmod 0755 "dist/Karamelo_linux"
...
linux_bin = "dist/Karamelo_linux"
linux_tar = f"dist/Karamelo_v{ver}_Linux64.tar.gz"
...
# Calcula hash e tamanho do binario Linux
bin_size = os.path.getsize(linux_bin) if os.path.exists(linux_bin) else 0
bin_sha = ""
...
# Campos do Linux
data["linux_bin_url"] = "https://karamelo-emu.com/downloads/Karamelo_linux"
data["linux_bin_size"] = bin_size
data["linux_bin_sha256"] = bin_sha
data["linux_tar_url"] = f"https://karamelo-emu.com/downloads/Karamelo_v{ver}_Linux64.tar.gz"

with open(json_path, "w", encoding="utf-8") as f:
    json.dump(data, f, indent=4, ensure_ascii=False)

print(f"  version.json atualizado com sucesso!")
print(f"  Linux Bin: {bin_size} bytes (SHA256: {bin_sha})")
print(f"  Linux Tar: {linux_tar}")
PYEOF

echo ""
echo "======================================================="
echo "  PACOTE LINUX GERADO COM SUCESSO"
echo "======================================================="
echo "  Pacote: dist/$TAR_FILE"
echo "  Updater: dist/Karamelo_linux"
