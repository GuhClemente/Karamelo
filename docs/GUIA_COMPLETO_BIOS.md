# 🕹️ Guia Oficial de BIOS e Arquivos de Sistema — MiSTer 4 ALL

Este guia detalha **todos os arquivos de BIOS, firmwares, chaves criptográficas e assets de sistema** necessários ou recomendados para os **35 sistemas emulados nativamente** no **MiSTer 4 ALL**.

---

## 📦 O Pacote Oficial de BIOS (`MiSTer_4_ALL_Pack_BIOS.zip`)

Para simplificar a configuração, criamos o pacote consolidado **`MiSTer_4_ALL_Pack_BIOS.zip`** com **toda a hierarquia de pastas correta** e os arquivos já conferidos (MD5/SHA-256).

### 🚀 Como Instalar o Pacote de BIOS:
1. Baixe o arquivo `MiSTer_4_ALL_Pack_BIOS.zip`.
2. Extraia o conteúdo diretamente na **pasta raiz da instalação do MiSTer 4 ALL** (onde fica o `MiSTer_4_ALL.exe`).
...
| **PlayStation 2 (PS2)** | `ps2.dll`<br>*(PCSX2 / LRPS2)* | `SCPH-70012_BIOS_V12_USA_200.BIN`<br>`SCPH-70012_BIOS_V12_USA_200.mec`<br>`SCPH-70012_BIOS_V12_USA_200.nvm`<br>`GameIndex.yaml` | `d333558cc14561c1fdc334c75d5f37b7`<br>`3faf7c064a4984f53e2ef5e80ed543bc`<br>`116b0e0f3d0ecef3997cf53266fa27a1`<br>*(banco de dados)* | `app/bios/` ou `app/bios/pcsx2/bios/` | BIOS real de 4MB (Slim v12) + NVRAM persistente. O MiSTer 4 ALL sincroniza automaticamente para o PCSX2. |
...
| **Commodore Amiga** | `amiga.dll`<br>*(PUAE)* | `kick34005.A500` *(Kickstart 1.3)*<br>`kick40060.CD32` *(Kickstart CD32)*<br>`kick40068.A1200` *(Kickstart 3.1)* | `app/bios/` | O core possui fallback AROS embutido, mas Kickstarts originais garantem fidelidade sonora do chip Paula e sincronia AGA. |
...
## 📂 Mapa de Pastas da Instalação do MiSTer 4 ALL

```text
MiSTer 4 ALL/
├── MiSTer_4_ALL.exe          <-- Executável principal
...
Abra o PowerShell na pasta do MiSTer 4 ALL e execute:
